    ...
    return DB::transaction(function () use ($request, $mapRateMode) {
       ...
       $boxSizesInput = $request->input('standard_boxes', []);
        ...
        foreach ($boxSizesInput as $b) {
            ...
            // Idempotent upsert on unique (Shippers_Id, Shippers_Box_Code)
            ShipperBoxSize::updateOrCreate(
                [
                    'Shippers_Id'       => $shipper->id,
            ...
            // Create a default box rate for each destination where boxes are enabled
            foreach ($destBlocks as $idx => $blk) {
                $destId = $createdDestIds[$idx] ?? null;
                if (!$destId) continue;
                ...
                foreach ($boxSizesInput as $b) {
                    ...
                    // If no default price provided, skip creating a rate (you can decide to create at 0.000 instead)
                    ...
                    ShipperBoxRate::create([
                        ...
                        'Shippers_Base_Fee'         => isset($b['Base_Fee']) ? (float)$b['Base_Fee'] : null, // optional if you add it in UI
                        'Shippers_Currency'         => $b['Currency'] ?? 'OMR',
                        'Shippers_Max_Weight_Kg'    => isset($b['Max_Weight_Kg']) ? (float)$b['Max_Weight_Kg'] : null, // per-rate override
                    ]);
                }
            }
    ...
    public function update(Request $request, int $id): JsonResponse
{
    ...
    return DB::transaction(function () use ($request, $shipper, $mapRateMode) {
        ...
        $boxSizeIdByCode = [];
        foreach ($boxSizesInput as $b) {
            $hasAny = array_filter($b, fn($v)=>!is_null($v) && $v!==''); if(!$hasAny) continue;
            $code  = trim((string)($b['Box_Code']  ?? ''));
            $label = trim((string)($b['Box_Label'] ?? ''));
            if ($code==='' || $label==='') continue;
        ...
        foreach ($destBlocks as $idx => $blk) {
            $destId = $createdDestIds[$idx] ?? null; if (!$destId) continue;
            $flags = $blk['flags'] ?? [];
